...
class generateResponse {
  ...
  private async init() {
    console.log(chalk.green(`开始生成接口文件，请稍后....`))
    ...
    let resStr = ''
    let apiSet: { name: string, url: string, method: 'get' | 'post' | 'delete' | 'put' }[] = []
    let apiSetType: {
      [key: string]: string
    } = {}
    let importApi = ''
    for (let item of this.allInterface) {
      ...
      if (result.success) {
        ...
        apiSet.push({
          name,
          url: item.path,
          method: item.method.toLowerCase() as "get" | "post" | "delete" | "put"
        })
        apiSetType[name] = `ApiNoop<${name}>`
        importApi += name + ' ,'
    ...
    let apiStr = String(readFileSync('./template.api.txt'))
    writeFileSync(cwd + '/index2.ts', apiStr.replace(/\$api/, JSON.stringify(apiSet)).replace(/\$interface/, importApi).replace(/\$type/, JSON.stringify(apiSetType).replace(/"(ApiNoop<(.*?)>)"/g, '$1')))
