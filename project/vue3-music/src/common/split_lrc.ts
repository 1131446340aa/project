function toLrc (Str: string) {
  var lrc: string[] = []
  var arrText: string[] = []
  var arrTime: string[] = []
  var arrDataTime: number[] = []
  lrc = Str.split('\n')
  var reg = /^\[+[0-2][0-4]:[0-5][0-9]\.[0-9]{2,}\]/
  for (let i = 0; i < lrc.length - 1; i++) {
    if (lrc[i].match(reg)) {
      arrTime.push((lrc[i].match(reg) as string[])[0])
      arrText.push(
        lrc[i]
          .split(reg)
          .join('')
  ...
  for (var item of arrTime) {
    arrDataTime.push(
      parseInt(
        String(Number(item.slice(1, item.length - 1).split(':')[0]) * 60)
      ) + parseInt(item.slice(1, item.length - 1).split(':')[1])
    )
  }
  return {
    arrText,
